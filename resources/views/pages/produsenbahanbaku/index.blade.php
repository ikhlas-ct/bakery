    ...
    <div class="container">
        ...
        <div class="ph-card mb-4">
            <div class="ph-left">
                ...
                <div>
                    <h5 class="ph-title">Bahan Baku Saya</h5>
                    <ol class="ph-breadcrumb mb-0">
                        <li><a href="{{ route('produsen.dashboard') }}">Dashboard</a></li>
                        <li><span class="bc-active">Bahan Baku Saya</span></li>
                    </ol>
                </div>
            </div>

            <a href="{{ route('produsen.bahan-baku.create') }}" class="btn btn-primary btn-sm">
            ...
            <div class="col-6 col-md-4">
                <div class="stat-card blue">
                    <div class="d-flex align-items-center gap-3">
                        ...
                        <div>
                            <div class="stat-value">{{ $totalDisediakan }}</div>
                            <div class="stat-label">Bahan Baku Disediakan</div>
                        </div>
                    </div>
                </div>
            </div>
                    ...
                    <div class="d-flex align-items-center gap-3">
                        ...
                        <div>
                            <div class="stat-value">
                                Rp{{ rtrim(rtrim(number_format($rataRataHarga ?? 0, 2, ',', '.'), '0'), ',') }}
                            </div>
                ...
                <div class="stat-card orange">
                    <div class="d-flex align-items-center gap-3">
                        ...
                        <div>
                            ...
                            <div class="stat-label">Kategori Berbeda</div>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="card filter-card">
            ...
            <div class="card-header d-flex align-items-center justify-content-between flex-wrap gap-2">
                <h5 class="mb-0"><i class="fas fa-list text-primary me-2"></i>Daftar Bahan Baku yang Saya Sediakan</h5>
                <span class="text-muted" style="font-size:.78rem;">{{ $bahanBakuProdusens->total() }} data ditemukan</span>
            </div>

            {{-- Filter --}}
            <div class="filter-section">
                <form method="GET" action="{{ route('produsen.bahan-baku.index') }}" class="row g-2 align-items-end">
                    ...
                    <div class="col-md-auto d-flex gap-2">
                        ...
                        @if (request()->hasAny(['search', 'kategori_barang_id']))
                            <a href="{{ route('produsen.bahan-baku.index') }}" class="btn btn-outline-secondary btn-sm px-3">
            ...
            <div class="table-responsive">
                <table class="table">
                    ...
                    <tbody>
                        @forelse ($bahanBakuProdusens as $i => $item)
                            <tr>
                                <td class="text-muted">{{ $bahanBakuProdusens->firstItem() + $i }}</td>
                                <td>
                                    <div class="name-text">{{ $item->bahanBaku->nama_bahan_baku ?? '-' }}</div>
                                    <span class="badge-kode">{{ $item->bahanBaku->kode_bahan_baku ?? '-' }}</span>
                                </td>
                                <td>
                                    <span class="badge-kategori">
                                        {{ $item->bahanBaku->kategoriBarang->nama_kategori ?? '-' }}
                                    </span>
                                </td>
                                <td>{{ $item->bahanBaku->satuan->nama ?? '-' }}</td>
                                <td>
                                    <span class="badge-harga">
                                        Rp{{ rtrim(rtrim(number_format($item->harga, 2, ',', '.'), '0'), ',') }}
                                    </span>
                                </td>
                                <td>
                                    <div class="d-flex gap-1">
                                        <a href="{{ route('produsen.bahan-baku.edit', $item->id) }}"
                                            class="btn btn-action btn-edit" title="Ubah Harga">
                                            <i class="fas fa-pencil-alt"></i>
                                        </a>
                                        <button class="btn btn-action btn-hapus" data-id="{{ $item->id }}"
                                            data-nama="{{ $item->bahanBaku->nama_bahan_baku ?? '' }}" title="Hapus">
                                            <i class="fas fa-trash-alt"></i>
                                        </button>
                                        <form id="form-hapus-{{ $item->id }}"
                                            action="{{ route('produsen.bahan-baku.destroy', $item->id) }}" method="POST"
                                            class="d-none">
                                            @csrf @method('DELETE')
                                        </form>
                                    </div>
                            ...
                            <tr>
                                <td colspan="6" class="p-0 text-center">
                                    <div class="empty-state">
                                        ...
                                        <div class="fw-semibold text-secondary mb-1">Belum ada bahan baku yang Anda sediakan</div>
                                        <div class="text-muted" style="font-size:.8rem;">
                                            Klik "Tambah Bahan Baku" untuk mulai memilih bahan baku yang bisa Anda pasok
                                        </div>
                                    </div>
                                </td>
                            </tr>
            ...
            {{-- Pagination --}}
            @if ($bahanBakuProdusens->hasPages())
                <div class="card-footer d-flex justify-content-between align-items-center flex-wrap gap-2">
                    <small class="text-muted">
                        Menampilkan
                        <strong>{{ $bahanBakuProdusens->firstItem() }}</strong>–<strong>{{ $bahanBakuProdusens->lastItem() }}</strong>
                        dari <strong>{{ $bahanBakuProdusens->total() }}</strong> data
                    </small>
                    {{ $bahanBakuProdusens->links() }}
                </div>
            @endif
    ...
    <script>
        document.querySelectorAll('.btn-hapus').forEach(btn => {
            btn.addEventListener('click', function () {
                ...
                swal({
                    title: 'Hapus Data?',
                    text: `Bahan baku "${nama}" akan dihapus dari daftar yang Anda sediakan.`,
                    icon: 'warning',
                    buttons: {
                        cancel: 'Batal',
