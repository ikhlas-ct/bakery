    ...
    <div class="container">
        ...
        <div class="ph-card mb-4">
            <div class="ph-left">
                ...
                <div>
                    <h5 class="ph-title">Data Kategori Barang</h5>
                    <ol class="ph-breadcrumb mb-0">
                        <li><a href="{{ $dashboardRoute }}">Dashboard</a></li>
                        <li><span class="bc-active">Kategori Barang</span></li>
                    </ol>
                </div>
            </div>
            ...
            <div class="col-6 col-md-4">
                <div class="stat-card green">
                    <div class="d-flex align-items-center gap-3">
                        ...
                        <div>
                            <div class="stat-value">{{ $kategoriTerpakai }}</div>
            ...
            <div class="col-6 col-md-4">
                <div class="stat-card orange">
                    <div class="d-flex align-items-center gap-3">
                        ...
                        <div>
                            <div class="stat-value">{{ $kategoriBelumTerpakai }}</div>
        ...
        <div class="card filter-card">
            ...
            <div class="card-header d-flex align-items-center justify-content-between flex-wrap gap-2">
                <h5 class="mb-0"><i class="fas fa-list text-primary me-2"></i>Daftar Kategori Barang</h5>
                <span class="text-muted" style="font-size:.78rem;">{{ $kategoriBarangs->total() }} data ditemukan</span>
            </div>

            {{-- Filter --}}
            <div class="filter-section">
                <form method="GET" action="{{ route('kategori-barang.index') }}" class="row g-2 align-items-end">
                    <div class="col-md-8 col-sm-8">
                        <div class="input-group input-group-sm">
                            ...
                            <input type="text" name="search" class="form-control border-start-0 ps-0"
                                style="border-radius:0 10px 10px 0;" placeholder="Cari nama kategori…"
                    ...
                    <div class="col-md-auto d-flex gap-2">
                        ...
                        @if (request()->hasAny(['search']))
                            <a href="{{ route('kategori-barang.index') }}" class="btn btn-outline-secondary btn-sm px-3">
            ...
            <div class="table-responsive">
                <table class="table">
                    ...
                    <tbody>
                        @forelse ($kategoriBarangs as $i => $kategori)
                            <tr>
                                <td class="text-muted">{{ $kategoriBarangs->firstItem() + $i }}</td>
                                <td><span class="name-text">{{ $kategori->nama_kategori }}</span></td>
                                <td>
                                    @if ($kategori->bahan_bakus_count > 0)
                                        <span class="badge-count">{{ $kategori->bahan_bakus_count }} bahan baku</span>
                                    @else
                                        <span class="badge-empty">Belum dipakai</span>
                                    @endif
                                </td>
                                <td>
                                    <div class="d-flex gap-1">
                                        ...
                                        <button class="btn btn-action btn-hapus" data-id="{{ $kategori->id }}"
                                            data-nama="{{ $kategori->nama_kategori }}" title="Hapus">
                                            <i class="fas fa-trash-alt"></i>
                                        </button>
                                        <form id="form-hapus-{{ $kategori->id }}"
                                            action="{{ route('kategori-barang.destroy', $kategori->id) }}" method="POST"
                                            class="d-none">
                                            @csrf @method('DELETE')
                                        </form>
                                    </div>
                            ...
                            <tr>
                                <td colspan="4" class="p-0 text-center">
                                    <div class="empty-state">
                                        ...
                                        <div class="fw-semibold text-secondary mb-1">Belum ada data kategori barang</div>
            ...
            {{-- Pagination --}}
            @if ($kategoriBarangs->hasPages())
                <div class="card-footer d-flex justify-content-between align-items-center flex-wrap gap-2">
                    <small class="text-muted">
                        Menampilkan
                        <strong>{{ $kategoriBarangs->firstItem() }}</strong>–<strong>{{ $kategoriBarangs->lastItem() }}</strong>
                        dari <strong>{{ $kategoriBarangs->total() }}</strong> data
                    </small>
