...
@section('title', 'Detail Produsen – ' . $produsen->nama_produsen)
...
<div class="container">
    ...
    <div class="ph-card">
        <div class="ph-left">
            ...
            <div>
                <h5 class="ph-title">Detail Produsen</h5>
                <ol class="ph-breadcrumb mb-0">
                    <li><a href="{{ route($dashboardRoute) }}">Dashboard</a></li>
                    <li><a href="{{ route($routePrefix . '.index') }}">Produsen</a></li>
                    <li><span class="bc-active">{{ $produsen->nama_produsen }}</span></li>
                </ol>
            </div>
        </div>
        <div class="d-flex gap-2">
            <a href="{{ route($routePrefix . '.edit', $produsen->id) }}" class="btn btn-warning btn-sm">
                <i class="fas fa-pencil-alt me-1"></i> Edit
            </a>
            <button type="button" class="btn btn-danger btn-sm" id="btn-hapus-detail">
                <i class="fas fa-trash-alt me-1"></i> Hapus
            </button>
            <form id="form-hapus-detail" action="{{ route($routePrefix . '.destroy', $produsen->id) }}" method="POST" class="d-none">
                @csrf @method('DELETE')
            </form>
    ...
    <div class="row g-4">
        ...
        <div class="col-lg-8">
            <div class="card info-card mb-4">
                <div class="card-header"><i class="fas fa-id-card text-primary me-2"></i>Data Produsen</div>
                <div class="info-row">
                    <div class="info-label"><i class="fas fa-industry"></i> Nama Produsen</div>
                    <div class="info-value">{{ $produsen->nama_produsen }}</div>
                </div>
                <div class="info-row">
                    <div class="info-label"><i class="fas fa-phone"></i> No. Telp</div>
                    <div class="info-value">{{ $produsen->no_telp ?: '-' }}</div>
                </div>
                <div class="info-row">
                    <div class="info-label"><i class="fas fa-map-marker-alt"></i> Alamat</div>
                    <div class="info-value">{{ $produsen->alamat ?: '-' }}</div>
                </div>
                <div class="info-row">
                    <div class="info-label"><i class="fas fa-handshake"></i> Status Mitra</div>
            ...
            <div class="card info-card mb-4">
                ...
                <div class="info-row">
                    ...
                    <div class="info-value">{{ $produsen->user->username }}</div>
                </div>
                <div class="info-row">
                    <div class="info-label"><i class="fas fa-envelope"></i> Email</div>
                    <div class="info-value">{{ $produsen->user->email }}</div>
                </div>
                <div class="info-row">
                    <div class="info-label"><i class="fas fa-user-tag"></i> Role</div>
                    <div class="info-value text-capitalize">{{ $produsen->user->role }}</div>
                </div>
                <div class="info-row">
                    <div class="info-label"><i class="fas fa-toggle-on"></i> Status Akun</div>
                    <div class="info-value">
                        @if ($produsen->user->is_active)
                            <span class="badge-aktif">Aktif</span>
                        @else
                            <span class="badge-nonaktif">Nonaktif</span>
                        @endif
                ...
                <div class="info-row">
                    ...
                    <div class="info-value">{{ $produsen->created_at?->translatedFormat('d F Y, H:i') ?? '-' }}</div>
                </div>
            </div>
        </div>
...
<script>
    document.getElementById('btn-hapus-detail').addEventListener('click', function () {
        swal({
            title: 'Hapus Data?',
            text: 'Data produsen "{{ $produsen->nama_produsen }}" akan dihapus permanen.',
            icon: 'warning',
            buttons: {
                cancel: 'Batal',
